...
int e01()
{
  ...
  else
  {
    ...
    sscanf(header, "matrix is: %dx%d", &row, &col);
...
int e03()
{
  FILE *f = fopen("fugitive_dump.bin", "rb");
  if (!f)
  {
    perror("bynari cant be oppen");
    return 1;
  }

  fseek(f, 20 + 10 + 4, SEEK_SET);
  short age;
  fread(&age, sizeof(short), 1, f);
  fclose(f);

  printf("the dugitive age is: %d\n", age);
  // I RAN THE PROGRAM, AND
  // THE AGE OF THE FUGUTIVE ON THIS DUMP IS %d
  return 0;
}
...
int e05()
{
  ...
  node **array = malloc(10 * sizeof(node *));
  for (int i = 0; i < 10; i++)
    array[i] = NULL;
