...
/* Exam Term 3: Andres Gomez Tagle Azcarraga*/
...
Can you explain in your own words why they are important and what they do?
  ...
  R= in define MAP_H_ you are givin a direct way of how the program should work, in ifndef, its a most ambiguous, where is a set of way

(5 PTS)
2)	Mention the differences betwen a SET and Linkedlist, from the point of their
    logical behaviour (not actual implementation)

  R= in aset you need to point exactly where the pointer is, in a linkedlist a pointer is directly into another pointer, this will move to the target and you dont need to know or specify the exact location of the memory
...
3)	Why do we implement a SET using binary tree as the underlying structure?
  ...
  R= we can make it faster ans save memory, due that we dont save the raw data in binary we can compress the infromation and still use it that way
...
5)	Mention the difference between a Priority Queue, and a regular queue, from
  ...
  R= regular queue will behave as a "i take the way it comes", when used the regular queue will not filter or modify where the data need to be, wen is used the data will leave the queue the same way it was inserted,
  when we use a priority queue, we need to specify how the queue is going to filter the data that is arriving, that way wen data came, it will take the information acording to your filter
...
void count_odd_nodes(node *root)
{
  if (root == NULL)
    return;
  ...
  count_odd_nodes(root->left);
  count_odd_nodes(root->right);
}
...
int main()
{
  ...
  count_odd_nodes(root);
  printf("Total odd nodes: %d\n", odd_count);
  ...
  map_insert(mapa, "Pollito1", &r1);
  map_insert(mapa, "Pollito2", &r2);
  map_insert(mapa, "Pollito3", &r3);

  store *s = (store *)map_get(mapa, "Pollito2");
  printf("Pollito2 lives in: %s\n", s->name);
